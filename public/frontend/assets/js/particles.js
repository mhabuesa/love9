...
// Get the value inside the <p> element with the class "sound"
const soundElement = document.querySelector(".sound");
const volume = soundElement ? parseInt(soundElement.textContent) : 0;
...
const configs = {
  ...
  backgroundMask: { enable: false, cover: { color: "#000" } },
  emitters: {
    direction: "top",
    life: { count: 0, duration: 0.1, delay: 0.2 },
  ...
  particles: {
    ...
    effect: {
      type: "trail",
      options: { trail: { length: { min: 10, max: 30 }, minWidth: 1, maxWidth: 1 } }
    },
    rotate: { path: true },
    shape: { type: "circle" },
    size: { value: 1 },
  ...
  sounds: {
    ...
    volume: document.getElementById("countdown").classList.contains("volume") ? volume : 0
}
};
